#!/usr/bin/env python3
"""
Générateur de flux RSS pour GitHub Actions
...
SITES = [
    {
        'url': 'https://www.facebook.com/groups/1074783704057108/',
        'output': 'FWU.xml',
        'name': 'FWU Life insurance Entraide Victimes'
    },
...
def extract_articles(soup, base_url):
    ...
    # Stratégie 3 : Chercher tous les h2/h3
    if not articles:
...
def generate_rss(site_config):
    """Génère un flux RSS pour un site"""
    url = site_config['url']
    output_file = site_config['output']
    site_name = site_config['name']
    
    print(f"\n📥 Génération du flux pour {site_name}...")
    print(f"   URL : {url}")
    ...
    try:
        ...
        SubElement(channel, 'description').text = f"Flux RSS généré automatiquement depuis {site_name}"
        ...
        print(f"   ✅ {len(articles)} articles extraits → {output_file}")
        return True
...
def main():
    ...
    # Retourner un code d'erreur si tout a échoué
    if success_count == 0:
        sys.exit(1)
